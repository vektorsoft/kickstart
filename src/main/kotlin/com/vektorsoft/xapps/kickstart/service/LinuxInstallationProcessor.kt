...
class LinuxInstallationProcessor {
    ...
    fun performInstall(config: RuntimeConfig, installDir : File) {
        val shortcutDir = findMenuShortcutDirectory()
        logger.debug("Found menu shortcut directory {}", shortcutDir.absolutePath)
        val entryData = createDesktopEntryContent(config, installDir)
        logger.debug("Desktop entry data: {}", entryData)
        File(shortcutDir, config.info.name.trim() + ".desktop").writeText(entryData, StandardCharsets.UTF_8)
    ...
    private fun createDesktopEntryContent(config : RuntimeConfig, installDir : File) : String {
        ...
        return content.replace("{app.name}", config.info.name)
                .replace("{app.desc}", "")
                .replace("{app.launcher}", File(installDir, config.jvm.launcher.fileName).absolutePath)
                .replace("{app.icon}", File(installDir, config.info.icon[0].fileName).absolutePath)
